...
describe('@repo/domain package exports', () => {
  it('resolves root and focused subpath exports', () => {
    ...
    expect(CommitmentOperationalActivityInputSchema.shape.lifecycleState).toBeDefined()
    expect(ClosingReadinessInputSchema.safeParse({ blockers: [] }).success).toBe(true)
    expect(CapitalReconciliationInputSchema.shape.targetAmountCents).toBeDefined()
